...
export const fetchSoleModuleProducts = () => ({
  type: SHOP.FETCH_SOLE_MODULE_PRODUCTS,
});
export const fetchSoleModuleProductsSuccess = (data) => ({
  type: SHOP.FETCH_SOLE_MODULE_PRODUCTS_SUCCESS,
  payload: {
    data,
  },
});

export const fetchSoleModuleProductsFail = (err) => ({
  type: SHOP.FETCH_SOLE_MODULE_PRODUCTS_FAIL,
  payload: {
    err,
  },
});
export const fetchSoleModuleProductsRequest = (query) => {
  return (dispatch) => {
    dispatch(fetchSoleModuleProducts());
    shopApis;
    fetchWooCommerceProducts(query)
      ...
      .catch((err) => {
        dispatch(fetchSoleModuleProductsFail(err));
      });
  };
};
...
//Get Sole product detail
export const fetchSoleProductDetail = () => ({
  type: SHOP.FETCH_SOLE_PRODUCT_DETAIL,
});

export const fetchSoleProductDetailSuccess = (data) => ({
  type: SHOP.FETCH_SOLE_PRODUCT_DETAIL_SUCCESS,
  payload: {
    data,
  },
});

export const fetchSoleProductDetailFail = (err) => ({
  type: SHOP.FETCH_SOLE_PRODUCT_DETAIL_FAIL,
  payload: {
    err,
  },
});

export const fetchSoleProductDetailRequest = (slug) => {
  return (dispatch) => {
    dispatch(fetchSoleProductDetail());
    shopApis
      .fetchSoleProductDetailData(slug)
      .then((res) => {
        dispatch(fetchSoleProductDetailSuccess(res.data));
      })
      .catch((err) => {
        dispatch(fetchSoleProductDetailFail(err));
      });
  };
};
